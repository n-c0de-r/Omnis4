...
func play_list(list: Array[int]) -> void:
	...
	if _is_rotating:
		...
		await rotate_buttons(ROTATE_DEGREES, direction, times, speed)
...
func rotate_buttons(degrees: int, direction: int, times: int, speed: float) -> void:
	for i in times:
		var rotate_tween: Tween = get_tree().create_tween()
		rotate_tween.tween_property(_buttons, "rotation_degrees",
		_buttons.rotation_degrees + (direction * degrees), speed / 2)
